...
<head>
    ...
    <script type="text/javascript">
        ...
        $(function () {
            ...
            $("#tree1").ligerTree({
                url: '../Data_crm/Param_SysParam.ashx?Action=GetApp&rnd=' + Math.random(),
                onSelect: onSelect,
                idFieldName: 'id',
            ...
            $("#maingrid").ligerGrid({
                ...
                url: "../Data_crm/Sys_Menu.ashx?Action=GetParams&parentid=-1",
                width: '100%',
                height: '100%',
                heightDiff: 0,
        ...
        function toolbar() {
            $.getJSON("../Data_crm/toolbar.ashx?Action=GetSys&mid=43&rnd=" + Math.random(), function (data, textStatus) {
                //alert(data);
                var items = [];
                var arr = data.Items;
        ...
        function onSelect(note) {
            ...
            var url = "../Data_crm/Param_SysParam.ashx?Action=GetParams&parentid=" + note.data.id + "&rnd=" + Math.random();
            ...
            if (row) {
                $.ligerDialog.confirm("删除后不能恢复，\n您确定要删除？", function (yes) {
                    if (yes) {
                        $.ajax({
                            type: "POST",
                            url: "../Data_crm/Param_SysParam.ashx",
                            data: { Action: 'del', paramid: row.id },
        ...
        function f_save(item, dialog) {
            ...
            if (issave) {
                ...
                $.ajax({
                    url: "../Data_crm/Param_SysParam.ashx", type: "POST",
                    data: issave,
