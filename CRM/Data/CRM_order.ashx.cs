        ...
        public void ProcessRequest(HttpContext context)
        {
            ...
            if (request["Action"] == "save")
            {
                ...
                for (int i = 0; i < postdata.Length; i++)
                {
                    ...
                    modeldel.base_price = postdata[i].Base_Price;
                    modeldel.amount = postdata[i].Amount;
                    modeldel.pway = postdata[i].Pway;
            ...
            if (request["Action"] == "pass")
            {
                ...
                model.Pass_status_id = int.Parse(request["T_Pass_status_val"]);
                model.Pass_status = PageValidate.InputText(request["T_Pass_status"], 255);
                model.Pass_details = PageValidate.InputText(request["T_Pass_details"], 255);
                model.Pass_date = DateTime.Now;
            ...
            if (request["Action"] == "grid")
            {
                ...
                if (!string.IsNullOrEmpty(request["pass"]))
                    serchtxt += " and Pass_status_id = " + int.Parse(request["pass"]);
            ...
            if (request["Action"] == "gridorder")
            {
                ...
                if (!string.IsNullOrEmpty(request["C_department"]))
                    //serchtxt += " and F_dep_id = " + int.Parse(request["department_val"]);

                if (!string.IsNullOrEmpty(request["C_employee"]))
                    serchtxt += " and Customer_id in (select id from CRM_Customer where Create_id=" + int.Parse(request["C_employee_val"]) + ")";
                ...
                {
                    serchtxt += " and tel like N'%" + PageValidate.InputText(request["tel"], 100) + "%'";
                    oSql = "1";
                }
                ...
                if (oSql == "1")
                {
                    ds = order.GetList(PageSize, PageIndex, serchtxt, sorttext, out Total, out oSql);
                }
                else
                {
                    ds = order.GetList(PageSize, PageIndex, serchtxt, sorttext, out Total);
                }
