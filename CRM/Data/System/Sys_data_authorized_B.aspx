...
<head>
    ...
    <script type="text/javascript">
        ...
        function loadform(role_id) {
            //../Data_crm/Sys_data_authority.ashx?Action=get&Role_id=' + getparastr("rid")
            $.ajax({
                type: "POST", dataType: 'json',
                url: "../Data_crm/Sys_data_authority.ashx", /* 注意后面的名字对应CS的方法名称 */
                ...
                success: function (result) {
                    ...
                    $("#task_view").ligerGetComboBoxManager().selectValue(data[4].Sys_view);
                    $("#task_del").ligerGetComboBoxManager().selectValue(data[4].Sys_del);
                }
            });
        }
        ...
        var data4 = [{ id: 1, text: '本人' }, { id: 2, text: '本部' }, { id: 3, text: '本部及下级' }];
        ...
        function initSelect() {
            ...
            f = $("#task_del").ligerComboBox({ width: 100 });
            ...
            $("#task_view").ligerComboBox({
                width: 100,
                data: data1,
                initValue: 1,
                onSelected: function (newvalue, newtext, newid) {
                    if (newvalue == 1) {
                        f.setData(data4); f.selectValue(1);
                    }
                    else if (newvalue == 2) {
                        f.setData(data3); f.selectValue(2);
                    }
                    else if (newvalue == 3) {
                        f.setData(data2); f.selectValue(3);
                    }
                    else {
                        f.setData(data1); f.selectValue(4);
                    }
        ...
        function f_save() {
            if ($("#form1").valid()) {
                ...
                postdata.push({
                    option_id: 5,
                    Sys_option: "任务管理",
                    Sys_view: $("#task_view_val").val(),
                    Sys_add: 3,
                    Sys_edit: 3,
                    Sys_del: $("#task_del_val").val()
                });
    ...
    <form id="form1">
        <div>
            <table style="width: 500px; margin: 5px;" class="bodytable0">
                ...
                <tr>
                    <td class="l-messagebox-btn-inner">任务管理</td>
                    <td class="l-messagebox-btn-inner">
                        <input type="text" id="task_view" name="task_view"></td>
                    <td class="l-messagebox-btn-inner">同查看</td>
                    <td class="l-messagebox-btn-inner">
                        <input type="text" id="task_del" name="task_del"></td>
                </tr>
            </table>
