    ...
    <script type="text/javascript">
        ...
        $(function () {
            ...
            $("#maingrid4").ligerGrid({
                ...
                url: "../Data_crm/hr_post.ashx?Action=grid&depid=0",
                width: '100%',
                height: '100%',
                heightDiff: -1
            });
        ...
        function toolbar() { 
            var items = [];
            items.push({ type: 'button', text: '新增岗位', icon: '../img_crm/icon/11.png', disable: true, click: function () { f_addpost() } });
            items.push({ type: 'textbox', id: 'empstatus' });
            items.push({ type: 'textbox', id: 'Serchtext', text: '岗位名：' });
            items.push({ type: 'button', text: '搜索', icon: '../img_crm/search.gif', disable: true, click: function () { f_search() } });
            ...
            $("#empstatus").ligerComboBox({
                ...
                onSelected: function (value, text) {
                    ...
                    if (note) {
                        ...
                        var url = "../Data_crm/hr_post.ashx?Action=grid&depid=" + note.data.id + "&empstatus=" + value + "&rnd=" + Math.random();
        ...
        function onSelect(note) {
            var manager = $("#maingrid4").ligerGetGridManager();
            manager.showData({ Rows: [], Total: 0 });
            var url = "../Data_crm/hr_post.ashx?Action=grid&depid=" + note.data.id + "&empstatus=" + $("#empstatus_val").val() + "&rnd=" + Math.random();
            manager.GetDataByURL(url);
        }
        ...
        function f_search() {
            ...
            var manager = $("#maingrid4").ligerGetGridManager();
            $.ajax({
                url: "../Data_crm/hr_post.ashx", type: "POST",
                data: serchtxt,
                dataType: 'json',
                beforeSend: function () {
                    manager.showData({ Rows: [], Total: 0 });
                ...
                success: function (responseText) {
                    //alert("../Data_crm/crm_customer.ashx" + serchtxt);
                    manager.setURL("../Data_crm/hr_post.ashx?" + serchtxt);
