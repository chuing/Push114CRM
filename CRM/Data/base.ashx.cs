    ...
    public class _base : IHttpHandler
    {
        ...
        public void ProcessRequest(HttpContext context)
        {
            ...
            if (request["Action"] == "GetIcons")
            {
                try
                {
                    string filetype = request["icontype"];

                    string rootPath = context.Server.MapPath("~/img_crm/icon/");
            ...
            if (request["Action"] == "GetFile")
            {
                try
                {
                    ...
                    if (string.IsNullOrEmpty(filetype))
                        filetype = "xls";
                    string rootFilePath = context.Server.MapPath("~/img_crm/impfile/" + filetype + "/");
                    Common.ObjectListToJSON objtojson = new Common.ObjectListToJSON();
                    List<FileInfo> lp = GetAllFilesInDirectory(rootFilePath);
                    string a = objtojson.toJSON(lp);
                    context.Response.Write(a);
