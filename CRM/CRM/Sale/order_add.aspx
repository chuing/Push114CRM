...
<head id="Head1">
    ...
    <script type="text/javascript">
        $(function () {
            ...
            $("#maingrid4").ligerGrid({
                columns: [
                        ...
                        { display: '产品名', name: 'product_name', width: 110 },
                        {
                            display: '单价', name: 'price', width: 84, type: 'float', align: 'right', editor: { type: 'text' }, render: function (item) {
                                return toMoney(item.price);
                            }
                        },
                        { display: '数量', name: 'quantity', width: 60, type: 'int', editor: { type: 'int', onChange: onChange } },
                        { display: '单位', name: 'unit', width: 80, editor: { type: 'text' } },
                        {
                        ...
                        {
                            display: '渠道', name: 'pway', width: 100, editor: { type: 'select', data: pwayData, valueField: 'id', textField: 'text' }
                        }
                ],
        ...
        {
            var pData = "";
            var _c = e.record.pway_content;
            if (_c != null && _c != "") {
                _c = _c.split(',');
                for (var i = 0; i < _c.length; i++) {
                    pData += "{id: '" + _c[i] + "', text: '" + _c[i] + "'},";
                }
                if (pData != "") { pData = pData.substring(0, pData.length - 1); }
            }
            //alert(_c);
            pData=eval('[' + pData + ']');
            e.column.editor.data = pData;
        ...
        function loadForm(oaid) {
            $.ajax({
                ...
                success: function (result) {
					...
					//$("#T_amount").ligerGetTextBoxManager().setDisabled();
