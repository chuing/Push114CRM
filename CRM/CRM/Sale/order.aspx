...
<head>
    ...
    <script type="text/javascript">
            ...
            $("#maingrid4").ligerGrid({
                columns: [
                   ...
                   },
                   {
                    ...
                    {
                        display: '审核状态', name: 'Pass_status_id', render: function (item) {
                            var html = "<div>"
                            if (item.Pass_status_id == "0")
                                html += "<span class='pass_n' onclick=\"n_show('" + item.id + "')\">未通过 查看原因</span>";
                            else if (item.Pass_status_id == "3")
                                html += "<span class='pass_n' onclick=\"n_show('" + item.id + "')\">作废 查看原因</span>";
                            else if (item.Pass_status_id == "1")
                                html += "<span class='pass_y'>已通过</span>";
                            else html += "<span class='pass_w'>审核中</span>";
                            html += "</div>";
                            return html;
                        }
                    },
                    { display: 'Oid', name: 'id', width: 80, align: 'left' }

                ],
                dataAction: 'server', pageSize: 30, pageSizeOptions: [20, 30, 50, 100],
                url: "../../Data_crm/CRM_order.ashx?Action=grid&rnd=" + Math.random(),
        ...
        var view_auth = false;
        function toolbar() {
            $.getJSON("../../Data_crm/toolbar.ashx?Action=GetSys&mid=34&rnd=" + Math.random(), function (data, textStatus) {
                //alert(data);
                var items = [];
                var arr = data.Items;
        ...
        function p_openWindow(url, title, width, height) {
            var dialogOptions = {
                width: width, height: height, title: title, url: url, buttons: [
                        {
                            text: '保存', onclick: p_save
                        },
                        {
                            text: '关闭', onclick: function (item, dialog) {
                                dialog.close();
        ...
        function pass() {
            ...
            var row = manager.getSelectedRow();
            if (row) {
                p_openWindow('CRM/sale/order_pass.aspx?orderid=' + row.id, "审核订单", 770, 530);
            }
            else {
                $.ligerDialog.warn('请选择行！');
            }
        ...
        function n_show(s) {
            parent.jQuery.ligerDialog.open({ width: 770, height: 530, title: "查看订单", url: "CRM/sale/order_pass.aspx?orderid=" + s, buttons: [{ text: '关闭', onclick: function (item, dialog) { dialog.close(); } }] });
        ...
        function del() {
            ...
            if (row) {
                $.ligerDialog.confirm("确定删除？", function (yes) {
                    if (yes) {
                        $.ajax({
                            url: "../../Data_crm/CRM_order.ashx", type: "POST",
    ...
    <style type="text/css">
        .passstatus {color:#f00; font-weight:bold; font-size:12px;}
        .pass_n {color:#999; font-weight:bold; cursor:pointer;}
        .pass_y {color:#00f; font-weight:bold;}
        .pass_w {color:#f00; font-weight:bold;}
    </style>
</head>
<body>
    <form id="form1">
