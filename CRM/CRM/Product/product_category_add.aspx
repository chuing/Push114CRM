...
<head>
    ...
    <script type="text/javascript">
        ...
        $(function () {
            ...
            if (getparastr("cid")) {
                ...
                $("#T_category_parent").ligerComboBox({
                    ...
                    treeLeafOnly: false,
                    tree: {
                        url: '../../Data_crm/Crm_product_category.ashx?Action=combo&rnd=' + Math.random(),
                        idFieldName: 'id',
        ...
        function loadForm(oaid) {
            $.ajax({
                type: "GET",
                url: "../../Data_crm/Crm_product_category.ashx", /* 注意后面的名字对应CS的方法名称 */
                ...
                $.ajax({
                    ...
                    success: function (data) {
                        ...
                        for (var i = 0, l = obj.length; i < l; i++) {
                            var src = obj[i];
                            var reg = /(img_crm\\icon)(.+)/;
                            var match = reg.exec(src);
                           // alert(match);
                            jiconlist.append("<li><img src='../../img_crm/icon/" + src.Name + "' /></li>");
                            if (!match) continue;
                        }
        ...
        $(".iconlist li").live('mouseover', function () {
            ...
            $("#menuicon").attr("src", "../../img_crm/icon" + name);
            $("#T_category_icon").val("img_crm/icon" + name);
