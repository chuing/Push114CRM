...
namespace XHD.BLL
{
	...
	public partial class CRM_order
	{
		...
		public List<XHD.Model.CRM_order> DataTableToList(DataTable dt)
		{
			...
			if (rowsCount > 0)
			{
				...
				for (int n = 0; n < rowsCount; n++)
				{
					...
					if(dt.Rows[n]["Delete_time"]!=null && dt.Rows[n]["Pass_date"].ToString()!="")
					{
						model.Delete_time=DateTime.Parse(dt.Rows[n]["Delete_time"].ToString());
                    }
                    if (dt.Rows[n]["Pass_status_id"] != null && dt.Rows[n]["Pass_status_id"].ToString() != "")
                    {
                        model.Pass_status_id = int.Parse(dt.Rows[n]["Pass_status_id"].ToString());
                    }
                    if (dt.Rows[n]["Pass_status"] != null && dt.Rows[n]["Pass_status"].ToString() != "")
                    {
                        model.Pass_status = dt.Rows[n]["Pass_status"].ToString();
                    }
                    if (dt.Rows[n]["Pass_details"] != null && dt.Rows[n]["Pass_details"].ToString() != "")
                    {
                        model.Pass_details = dt.Rows[n]["Pass_details"].ToString();
                    }
                    if (dt.Rows[n]["Pass_date"] != null && dt.Rows[n]["Pass_date"].ToString() != "")
                    {
                        model.Pass_date = DateTime.Parse(dt.Rows[n]["Pass_date"].ToString());
                    }
        ...
        {
            return dal.GetList(PageSize, PageIndex, strWhere, filedOrder, out Total, out oSql);
        }
        /// <summary>
        /// 更新发票金额
        /// </summary>
