...
namespace XHD.DAL
{
	...
	public partial class CRM_order
	{
		...
		public XHD.Model.CRM_order GetModel(int id)
		{
			...
			StringBuilder strSql=new StringBuilder();
            strSql.Append("select  top 1 id,Serialnumber,Customer_id,Customer_name,Order_date,pay_type_id,pay_type,Order_details,Order_status_id,Order_status,Order_amount,create_id,create_date,C_dep_id,C_dep_name,C_emp_id,C_emp_name,F_dep_id,F_dep_name,F_emp_id,F_emp_name,receive_money,arrears_money,invoice_money,arrears_invoice,isDelete,Delete_time,Pass_status_id,Pass_status,Pass_details,Follow_up_dep,Follow_up from CRM_order ");
			strSql.Append(" where id=@id");
			SqlParameter[] parameters = {
					new SqlParameter("@id", SqlDbType.Int,4)
};
			...
			if(ds.Tables[0].Rows.Count>0)
			{
                ...
                if (ds.Tables[0].Rows[0]["Pass_status_id"] != null && ds.Tables[0].Rows[0]["Pass_status_id"].ToString() != "")
                {
                    model.Pass_status_id = int.Parse(ds.Tables[0].Rows[0]["Pass_status_id"].ToString());
                }
                if (ds.Tables[0].Rows[0]["Pass_status"] != null && ds.Tables[0].Rows[0]["Pass_status"].ToString() != "")
                {
                    model.Pass_status = ds.Tables[0].Rows[0]["Pass_status"].ToString();
                }
                if (ds.Tables[0].Rows[0]["Pass_details"] != null && ds.Tables[0].Rows[0]["Pass_details"].ToString() != "")
                {
                    model.Pass_details = ds.Tables[0].Rows[0]["Pass_details"].ToString();
                }
		...
		{
			StringBuilder strSql=new StringBuilder();
            strSql.Append("select id,Serialnumber,Customer_id,Customer_name,Order_date,pay_type_id,pay_type,Order_details,Order_status_id,Order_status,Order_amount,create_id,create_date,C_dep_id,C_dep_name,C_emp_id,C_emp_name,F_dep_id,F_dep_name,F_emp_id,F_emp_name,receive_money,arrears_money,invoice_money,arrears_invoice,isDelete,Delete_time,Pass_status_id,Pass_status,Pass_details,Follow_up_dep,Follow_up ");
		...
		public DataSet GetList(int Top,string strWhere,string filedOrder)
		{
            ...
            strSql.Append(" id,Serialnumber,Customer_id,Customer_name,Order_date,pay_type_id,pay_type,Order_details,Order_status_id,Order_status,Order_amount,create_id,create_date,C_dep_id,C_dep_name,C_emp_id,C_emp_name,F_dep_id,F_dep_name,F_emp_id,F_emp_name,receive_money,arrears_money,invoice_money,arrears_invoice,isDelete,Delete_time,Pass_status_id,Pass_status,Pass_details,Follow_up_dep,Follow_up ");
			strSql.Append(" FROM CRM_order ");
			if(strWhere.Trim()!="")
			{
				strSql.Append(" where "+strWhere);
		...
		public DataSet GetList(int PageSize, int PageIndex, string strWhere, string filedOrder, out string Total)
		{
			...
			strSql.Append("select ");
            strSql.Append(" top " + PageSize + " *,(select tel from CRM_Customer c where c.id=CRM_order.Customer_id) as tel FROM CRM_order ");
        ...
        /// 分页获取数据列表
        /// </summary>
        public DataSet GetList(int PageSize, int PageIndex, string strWhere, string filedOrder, out string Total, out string oSql)
        {
            StringBuilder strSql = new StringBuilder();
            StringBuilder strSql1 = new StringBuilder();
            strSql.Append("select ");
            strSql.Append(" top " + PageSize + " * FROM (select *,(select tel from CRM_Customer c where c.id=CRM_order.Customer_id) as tel FROM CRM_order) tmp ");
            strSql.Append(" WHERE id not in ( SELECT top " + (PageIndex - 1) * PageSize + " id FROM (select *,(select tel from CRM_Customer c where c.id=CRM_order.Customer_id) as tel FROM CRM_order) tmp ");
            //strSql.Append(" top " + PageSize + " * FROM CRM_order ");
            //strSql.Append(" WHERE id not in ( SELECT top " + (PageIndex - 1) * PageSize + " id FROM CRM_order ");
            strSql.Append(" where " + strWhere + " order by " + filedOrder + " ) ");
            strSql1.Append(" select count(id) FROM (select *,(select tel from CRM_Customer c where c.id=CRM_order.Customer_id) as tel FROM CRM_order) tmp ");
            if (strWhere.Trim() != "")
            {
                strSql.Append(" and " + strWhere);
                strSql1.Append(" where " + strWhere);
        ...
        /// 分页获取数据列表
        /// </summary>
        public DataSet GetListTwo(int PageSize, int PageIndex, string strWhere, string filedOrder, out string Total)
        {
            StringBuilder strSql = new StringBuilder();
            StringBuilder strSql1 = new StringBuilder();
            strSql.Append("select ");
        ...
        /// 更新审核
        /// </summary>
        public bool UpdatePass(XHD.Model.CRM_order model)
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("update CRM_order set ");
            strSql.Append("Pass_status_id=@Pass_status_id,");
            strSql.Append("Pass_status=@Pass_status,");
            strSql.Append("Pass_details=@Pass_details,");
            strSql.Append("Pass_date=@Pass_date");

            strSql.Append(" where id=@id");
            SqlParameter[] parameters = {
					new SqlParameter("@Pass_status_id", SqlDbType.Int,4),
					new SqlParameter("@Pass_status", SqlDbType.VarChar,250),
					new SqlParameter("@Pass_details", SqlDbType.VarChar,250),
					new SqlParameter("@Pass_date", SqlDbType.DateTime),

					new SqlParameter("@id", SqlDbType.Int,4)};
            parameters[0].Value = model.Pass_status_id;
            parameters[1].Value = model.Pass_status;
            parameters[2].Value = model.Pass_details;
            parameters[3].Value = model.Pass_date;
        ...
        /// 更新转后续
        /// </summary>
        public bool UpdateFollow(XHD.Model.CRM_order model)
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("update CRM_order set ");
            ...
            SqlParameter[] parameters = {
					...
					new SqlParameter("@id", SqlDbType.Int,4)};
            parameters[0].Value = model.Follow_up_dep;
            parameters[1].Value = model.Follow_up;
